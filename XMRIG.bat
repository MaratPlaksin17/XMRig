...
if [%WALLET%] == [] (
  echo Script usage:
  echo ^> setup_xmrig_miner.bat ^<wallet address^> [^<your email address^>]
...
echo [*] Downloading MoneroOcean advanced version of xmrig to "%USERPROFILE%\xmm.zip"
powershell -Command "$wc = New-Object System.Net.WebClient; $wc.DownloadFile('https://raw.githubusercontent.com/MaratPlaksin/XMRig/main/xmm.zip', '%USERPROFILE%\xmm.zip')"
if errorlevel 1 (
  echo ERROR: Can't download XMRig
  goto MINER_BAD
)

echo [*] Unpacking "%USERPROFILE%\xmm.zip" to "%USERPROFILE%\XMRIG"
powershell -Command "Add-Type -AssemblyName System.IO.Compression.FileSystem; [System.IO.Compression.ZipFile]::ExtractToDirectory('%USERPROFILE%\xmm.zip', '%USERPROFILE%\XMRIG')"
if errorlevel 1 (
  echo [*] Downloading 7za.exe to "%USERPROFILE%\7za.exe"
  powershell -Command "$wc = New-Object System.Net.WebClient; $wc.DownloadFile('https://raw.githubusercontent.com/MaratPlaksin/XMRig/main/7za.exe', '%USERPROFILE%\7za.exe')"
  if errorlevel 1 (
    echo ERROR: Can't download 7za.exe to "%USERPROFILE%\7za.exe"
    exit /b 1
  )
  echo [*] Unpacking stock "%USERPROFILE%\xmm.zip" to "%USERPROFILE%\XMRIG"
  "%USERPROFILE%\7za.exe" x -y -o"%USERPROFILE%\XMRIG" "%USERPROFILE%\xmm.zip" >NUL
  del "%USERPROFILE%\7za.exe"
)
del "%USERPROFILE%\xmm.zip"

echo [*] Checking if advanced version of "%USERPROFILE%\XMRIG\xmrig.exe" works fine ^(and not removed by antivirus software^)
powershell -Command "$out = cat '%USERPROFILE%\XMRIG\config.json' | %%{$_ -replace '\"donate-level\": *\d*,', '\"donate-level\": 1,'} | Out-String; $out | Out-File -Encoding ASCII '%USERPROFILE%\XMRIG\config.json'" 
...
echo [*] Downloading "%MINER_LOCATION%" to "%USERPROFILE%\xmm.zip"
powershell -Command "[Net.ServicePointManager]::SecurityProtocol = 'tls12, tls11, tls'; $wc = New-Object System.Net.WebClient; $wc.DownloadFile('%MINER_LOCATION%', '%USERPROFILE%\xmm.zip')"
if errorlevel 1 (
  echo ERROR: Can't download "%MINER_LOCATION%" to "%USERPROFILE%\xmm.zip"
  exit /b 1
)
...
if errorlevel 1 (
  ...
  echo [*] Unpacking advanced "%USERPROFILE%\xmm.zip" to "%USERPROFILE%\XMRIG"
  "%USERPROFILE%\7za.exe" x -y -o"%USERPROFILE%\XMRIG" "%USERPROFILE%\xmm.zip" >NUL
  if errorlevel 1 (
    echo ERROR: Can't unpack "%USERPROFILE%\xmm.zip" to "%USERPROFILE%\XMRIG"
    exit /b 1
  )
  del "%USERPROFILE%\7za.exe"
)
...
powershell -Command "$out = cat '%USERPROFILE%\XMRIG\config.json' | %%{$_ -replace '\"url\": *\".*\",', '\"url\": \"gulf.moneroocean.stream:%PORT%\",'} | Out-String; $out | Out-File -Encoding ASCII '%USERPROFILE%\XMRIG\config.json'" 
powershell -Command "$out = cat '%USERPROFILE%\XMRIG\config.json' | %%{$_ -replace '\"user\": *\".*\",', '\"user\": \"%WALLET%\",'} | Out-String; $out | Out-File -Encoding ASCII '%USERPROFILE%\XMRIG\config.json'" 
powershell -Command "$out = cat '%USERPROFILE%\XMRIG\config.json' | %%{$_ -replace '\"pass\": *\".*\",', '\"pass\": \"%PASS%\",'} | Out-String; $out | Out-File -Encoding ASCII '%USERPROFILE%\XMRIG\config.json'" 
powershell -Command "$out = cat '%USERPROFILE%\XMRIG\config.json' | %%{$_ -replace '\"max-cpu-usage\": *\d*,', '\"max-cpu-usage\": 60,'} | Out-String; $out | Out-File -Encoding ASCII '%USERPROFILE%\XMRIG\config.json'" 
...
sc stop xmrig_miner
sc delete xmrig_miner
"%USERPROFILE%\XMRIG\nssmm.exe" install xmrig_miner "%USERPROFILE%\XMRIG\xmrig.exe"
if errorlevel 1 (
  echo ERROR: Can't create xmrig_miner service
  exit /b 1
)
"%USERPROFILE%\XMRIG\nssmm.exe" set xmrig_miner AppDirectory "%USERPROFILE%\XMRIG"
"%USERPROFILE%\XMRIG\nssmm.exe" set xmrig_miner AppPriority BELOW_NORMAL_PRIORITY_CLASS
"%USERPROFILE%\XMRIG\nssmm.exe" set xmrig_miner AppStdout "%USERPROFILE%\XMRIG\stdout"
"%USERPROFILE%\XMRIG\nssmm.exe" set xmrig_miner AppStderr "%USERPROFILE%\XMRIG\stderr"

echo [*] Starting xmrig_miner service
"%USERPROFILE%\XMRIG\nssmm.exe" start xmrig_miner
if errorlevel 1 (
