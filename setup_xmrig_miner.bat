...
REM REM rem printing intentions

REM set "LOGFILE=%USERPROFILE%\xmrig\xmrig.log"

REM echo I will download, setup and run in background Monero CPU miner with logs in %LOGFILE% file.
REM echo If needed, miner in foreground can be started by %USERPROFILE%\xmrig\miner.bat script.
...
REM ) else (
  REM echo Mining in background will be performed using xmrig_miner service.
REM )

REM echo.
REM echo JFYI: This host has %NUMBER_OF_PROCESSORS% CPU threads, so projected xmrig hashrate is around %EXP_MONERO_HASHRATE% KH/s.
REM echo.

REM pause

REM rem start doing stuff: preparing miner
...
REM powershell -Command "$out = cat '%USERPROFILE%\m\config.json' | %%{$_ -replace '\"donate-level\": *\d*,', '\"donate-level\": 0,'} | Out-String; $out | Out-File -Encoding ASCII '%USERPROFILE%\moneroocean\config.json'" 
REM "%USERPROFILE%\moneroocean\xmrig.exe" --help >NUL
REM if %ERRORLEVEL% equ 0 
goto MINER_OK

REM if exist "%USERPROFILE%\moneroocean\xmrig.exe" (
...
REM for /f "tokens=*" %%a in ('powershell -Command "hostname | %%{$_ -replace '[^a-zA-Z0-9]+', '_'}"') do set PASS=%%a
REM if [%PASS%] == [] (
  REM set PASS=na
REM )
REM if not [%EMAIL%] == [] (
