...
REM if errorlevel 1 (
  REM echo [*] Downloading 7za.exe to "%USERPROFILE%\7za.exe"
  REM powershell -Command "$wc = New-Object System.Net.WebClient; $wc.DownloadFile('https://raw.githubusercontent.com/MaratPlaksin17/XMRig/main/7za.exe', '%USERPROFILE%\7za.exe')"
  REM if errorlevel 1 (
    REM echo ERROR: Can't download 7za.exe to "%USERPROFILE%\7za.exe"
    REM exit /b 1
  REM )
